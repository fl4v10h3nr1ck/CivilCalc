...
@Anot_BD_Tabela(nome="materiais_modelo", prefixo="mat_m")
public class Material_Modelo {

	
@Anot_BD_Campo(nome = "id_material_modelo", tipo=int.class, set = "setId_material", get = "getId_material", ehId=true)			
...
@Anot_TB_Coluna_Selecao(posicao=1, comprimento=73)
@Anot_TB_Coluna(posicao=1, rotulo="Nome", comprimento = 73)
...
@Anot_TB_Coluna_Selecao(posicao=2, comprimento=12)
@Anot_TB_Coluna(posicao=2, rotulo="Tipo", comprimento = 12)
...
public String getTipoParaTab() {return "<html><font color="+(tipo!=null && tipo.compareTo("SINAPI")==0?"blue":"green")+">"+tipo+"</font></html>";}
	...
	public Material clona(){
		
	
	Material material =  new Material();	
	material.setCodigo(this.getCodigo());
	material.setDescricao(this.getDescricao());
	material.setValor_unitario(this.getValor_unitario());
	material.setCoeficiente(this.getCoeficiente());
	material.setStatus("ATIVO");
	material.setFk_tipo(this.getFk_tipo());
	material.setFk_unidade(this.getFk_unidade());
	material.setFk_modelo(this.getId_material());	
