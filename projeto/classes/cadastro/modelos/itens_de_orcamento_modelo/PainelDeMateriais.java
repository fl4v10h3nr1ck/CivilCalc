...
private JButton bt_novo_material;
	...
	public PainelDeMateriais(ItemDeOrcamentoModelo item, FormItensDeOrcamentoModelo principal) {
	...
	Material_Modelo material = new DAO<Material_Modelo>(Material_Modelo.class).get(this.idTempMaterial);	
		...
		if(material!= null){
			...
			for(Item aux: this.lista_materiais){
				...
				if(Integer.parseInt(aux.getParamentro("id_material").toString())==this.idTempMaterial){
	...
	return false;
		...
		if(this.item.getSem_insumos()==null || this.item.getSem_insumos().compareTo("SIM")!=0){
			...
			ItemXMaterialModelo	itemXmaterial = new ItemXMaterialModelo();
			
			itemXmaterial.setFk_material(Integer.parseInt(aux.getParamentro("id_material").toString()));
			itemXmaterial.setFk_item(this.item.getId_item_modelo());
			itemXmaterial.setQuantidade(aux.getParamentro("quant").toString());
			
			if(new DAO<ItemXMaterialModelo>(ItemXMaterialModelo.class).novo(itemXmaterial)<=0)
			return false;
			}	
	...
	this.lista_materiais.clear();
		...
		List<ItemXMaterialModelo> itensXMateriais = new DAO<ItemXMaterialModelo>(ItemXMaterialModelo.class)
					.get(null, "ixm_mod.fk_item="+this.item.getId_item_modelo(), null);

		DAO<Material_Modelo> materialDAO = new DAO<Material_Modelo>(Material_Modelo.class);
		DAO<Unidade> unidadeDAO = new DAO<Unidade>(Unidade.class);

			for(ItemXMaterialModelo aux : itensXMateriais){

			Material_Modelo material = materialDAO.get(aux.getFk_material());
				...
				System.out.println(Calculo.formataValor(aux.getQuantidade())+" "+material.getValor_unitario());
